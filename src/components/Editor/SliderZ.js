...
const SliderZ = (props) => {
  ...
  const handleDrag = useCallback((_, { offset }) => {
    ...
    sendMessage(objectName, "ChangeZPosition", parseFloat(displayValue.get().toFixed(2)));
  }, [objectName]);
  ...
  useEffect(() => {
    const checkZValue = () => {
      if (!isEditing && x.get() === 0 && objectName !== '') { // isEditing değeri false ve x.get() değeri 0 ise
        ...
        sendMessage(objectName, "SendZCoordToReact");
      }
    };
  ...
  const handleInputChange = (e) => {
    ...
    if (value.trim() !== "") { 
      sendMessage(objectName, "ChangeZPosition", parseFloat(value)); 
    }
    console.log(value);
  }
